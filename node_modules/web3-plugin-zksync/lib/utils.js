...
const checkReceipt = (hash, receipt) => {
    if (!receipt) {
        throw new errors_1.ReceiptError(`Transaction with hash ${hash} not found`, {
            hash,
        });
    }
    if (Number(receipt.status) === 0) {
        throw new errors_1.ReceiptError(`Transaction ${hash} failed with status 0`, {
            hash,
            receipt,
        });
    }
...
async function waitTxReceipt(web3Eth, txHash) {
    const receipt = await (0, web3_eth_1.waitForTransactionReceipt)(web3Eth, txHash, web3_1.DEFAULT_RETURN_FORMAT, customGetTransactionReceipt);
...
const getPriorityOpL1Response = async (context, l1TxPromise, contextL2) => {
    ...
    return {
        ...
        waitFinalize: async () => {
            ...
            const l2Hash = await contextL2.getL2TransactionFromPriorityOp(l1Receipt);
            if (!contextL2) {
                return {
