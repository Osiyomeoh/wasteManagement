...
function makeUpgradeBeacon(hre) {
    return async function upgradeBeacon(wallet, beaconImplementation, newImplementationArtifact, opts = {}, quiet = false) {
        const factory = new zk.ContractFactory(newImplementationArtifact.abi, newImplementationArtifact.bytecode, wallet, opts.deploymentType);
        ...
        const { impl: nextImpl } = await (0, deploy_impl_1.deployBeaconImpl)(hre, factory, opts, beaconImplementationAddress);
        if (!quiet) {
            console.info(chalk_1.default.green('New beacon impl deployed at', nextImpl));
        }
        const upgradableBeaconPath = (await hre.artifacts.getArtifactPaths()).find((x) => x.includes(path_1.default.sep + (0, utils_1.getUpgradableContracts)().UpgradeableBeacon + path_1.default.sep + constants_1.UPGRADABLE_BEACON_JSON));
